...
const Party: React.FC<PartyProps> = ({ guests, capacity }) => {
  ...
  return (
    <div style={partyAreaStyle}>
      <h3>Party Guests ({guests.length}/{capacity}) | Trouble: {troubleCount}/3</h3>
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {guests.map((card, index) => (
          <div key={index} style={{...cardStyle, borderColor: card.trouble ? 'red' : '#ccc'}}>
            <strong>{card.name}</strong>
            <span>Pop: {card.popularity} / Mon: {card.money}</span>
